% DISCRETE_ATTRACTORS.m generates the discrete attractor dynamical systems 
% of EDF 1 and EDF 9 in:
...
switch network_str
    ...
    case 'two_hemi_ramp'
        ...
        switch ramp_type
            ...
            otherwise
                disp('ERROR: no valid paratemer for ramping has been selected')
                return               
        end
    ...
    otherwise
        disp('ERROR: no valid network structure has been selected')
        return
...
stim_smooth_vec = my_expsmooth(stim_t, fs, tau_ext);
...
PV_smooth_vec = my_expsmooth(stim_t, fs, tau_ext);
...
[correct_trials_right, correct_trials_left, r_right_pj_c, r_left_pj_c] = ALM_attractor_figs(r_mat, ...
